...
This is my final project for CS 201. The project is a Book Application system that stores and keeps track of books a user has read. The program runs in the command line and gives the user a menu of options to choose from.

The application works with two types of books: printed books and audiobooks.
...
The program also calculates the cost of each book based on the project requirements.

Printed books cost $0.50 per page.
Audiobooks cost $0.25 per minute.

The program allows the user to add books, search books, sort books, display statistics, display last-N book records, save book data to a file, and load book data from a file.
...
This helps demonstrate file persistence. The user can save the book list, clear the data, reload the saved file, and verify that the book information is still correct.

Design Explanation

The project uses an interface, an abstract class, two subclasses, an application class, a file manager class, a main class, and a test case class.
...
Calculating the total cost of all stored books
Book
...
The Book class stores the common information for all books:
...
The provided sample file already has 10 books, so I used that file first. Then I added the extra required edge case and large value books because those were not included in the sample file.

Notes

I made the project a command-line application because that matches the style of several examples from class. I used a simple numbered menu and a Y/N continue question after each action.

I also added comments in the code to explain the main sections and where the object-oriented programming concepts are being used.
